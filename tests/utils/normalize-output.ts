...
function replacePathLike(input: string, target: string, replacement: string): string {
...
export function normalizeOutput(str: string, cwd: string, root: string): string {
  ...
  return (
    normalized
      ...
      // Node 18 diffs
      .replaceAll("  ErrorCaptureStackTrace(err);", "validateFunction(listener, 'listener');")
      ...
      .replaceAll(/(?:file:\/\/\/|file:\/\/)?\/?(?:[A-Z]:)?(?:\/[\w.-]+)+/gi, normalizeAbsolutePath)
      .trim()
  );
}
