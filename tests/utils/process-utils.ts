...
export async function waitForPidFile(pidFilePath: string, timeoutMs = 5_000): Promise<number> {
...
export async function waitForProcessTermination(pid: number, timeoutMs = 5_000): Promise<void> {
  const start = Date.now();
