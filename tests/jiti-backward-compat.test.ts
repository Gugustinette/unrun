...
describe.concurrent("backward compatibility with jiti", () => {
  for (const fixture of fixtures) {
    ...
    test(fixture, { timeout: 20000 }, async () => {
      ...
      await captureConsole(async () => {
        ...
        unrunModule = (await unrun({ path: fixturePath, preset: "jiti" })).module;
      });
  ...
  test("error-parse throws", async () => {
    const fixturePath = resolve(__dirname, "fixtures/jiti", "./error-parse/index.ts");

    await expect(captureConsole(() => jiti.import(fixturePath))).rejects.toThrow();
    await expect(captureConsole(() => unrun({ path: fixturePath }))).rejects.toThrow();
  });

  // Error at runtime
  test("error-runtime throws", async () => {
    const fixturePath = resolve(__dirname, "fixtures/jiti", "./error-runtime/index.ts");
